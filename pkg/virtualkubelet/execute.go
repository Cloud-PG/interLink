...
// qui va una funzione per fare il mock: runRequest(jsonInput struct) outPut struct
func runRequest(jsonBody []byte) []byte {
	request := jsonRequest{}
	var returnValue, _ = json.Marshal(outPut{"Error", "Error", ERROR})
	...
	returnValue, _ = json.Marshal(outPut{"Create Pod", string(respBody), CREATE})

	return returnValue
}

func killRequest(jsonBody []byte) []byte {
	request := jsonRequest{}
	var returnValue, _ = json.Marshal(outPut{"Error", "Error", ERROR})
	json.Unmarshal(jsonBody, &request)
	...
	respBody, _ := ioutil.ReadAll(resp.Body)
	returnValue, _ = json.Marshal(outPut{"Delete Pod", string(respBody), DELETE})
...
func getStatus(jsonBody []byte) []byte {
	...
	respBody, _ := ioutil.ReadAll(resp.Body)
	returnValue, _ = json.Marshal(outPut{"Get Pod Status", string(respBody), STATUS})

	return returnValue
}

// poi la chiami qui dentro
...
// qui va una funzione per fare il mock: killRequest(jsonInputKill struct) outPutKill struct
...
func RemoteExecution(p *VirtualKubeletProvider, ctx context.Context, mode int8, imageLocation string, pod *v1.Pod, container v1.Container) error {
	var err error
	var jsonVar jsonRequest
	//instance_name := BuildRemoteExecutionInstanceName(container, pod)
	// // QUI CI SARA' LA CONFIGURAZIONE PER LA CONNESSIONE AL SERVIZIO REMOTO
	...
	case common.CREATE:
		...
		jsonVar = jsonRequest{*pod}
		jsonBytes, _ := json.Marshal(jsonVar)
		returnVal := runRequest(jsonBytes)
	...
	case common.DELETE:
		...
		jsonVar = jsonRequest{*pod}
		jsonBytes, _ := json.Marshal(jsonVar)
		returnVal := killRequest(jsonBytes)
		log.G(ctx).Infof(string(returnVal))
...
func PrepareContainerData(p *VirtualKubeletProvider, ctx context.Context, client *simplessh.Client, instance_name string, container v1.Container, pod *v1.Pod) error {
...
// ALTRO PUNTO DA IMPLEMENTARE CON MOCK
func checkPodsStatus(p *VirtualKubeletProvider, ctx context.Context) {
	...
	var jsonBytes []byte
	var returnVal []byte
	var podsList statusRequest

	//out, _ := exec.Command("kubectl", "get", "pods").Output()
	//log.G(ctx).Infof(string(out))

	for _, pod := range p.pods {
		podsList.Pods = append(podsList.Pods, jsonRequest{*pod})
	}
	...
	// 			if lastStatus.Ready {
	// 				// QUI SOSTITUISCI CON MOCK E DOVREBBE ANDARE TUTTO OK. Per ora metti 0 fisso
